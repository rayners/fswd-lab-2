...
server.post('/tasks/:task_id/complete', function(request, response) {
    if (request.task.canUserComplete(request.user)) {
...
server.post('/tasks', function(request, response) {
    ...
    if (request.user) {
        models.Task.create({ name: task }).then(function(newTask) {
            response.format({
                html: function() {
                    response.redirect('/tasks');
                },
        ...
        response.status(403).json({ error: 'Must be logged in to create a task' });
    }
